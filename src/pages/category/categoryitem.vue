...
<template>
    <div class="flex items-center space-x-3 border-b hover:bg-gray-50 duration-300">
        <div class="p-2 w-52 flex space-x-1 items-center cursor-pointer group" @click.stop="emit('changeMenu',item)">
            ...
            <div v-else>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" class="fill-gray-500"><path d="M20 22H4C3.44772 22 3 21.5523 3 21V3C3 2.44772 3.44772 2 4 2H20C20.5523 2 21 2.44772 21 3V21C21 21.5523 20.5523 22 20 22ZM7 6V10H11V6H7ZM7 12V14H17V12H7ZM7 16V18H17V16H7ZM13 7V9H17V7H13Z"></path></svg>
            </div>
        ...
        <div class="p-2 w-36">{{ item.pinyin||'未定义' }}</div>
        <div class="p-2 flex-1">
            <ypswitch v-model="item.nav" @click="categoryInject.update(item)"></ypswitch>
        </div>
        <div class="p-2 w-36 flex space-x-2 justify-end">
          <button class="yp-button yp-button-gray yp-button-sm rounded" @click="categoryInject.editItem(item.id)">编辑</button>
          <button class="yp-button yp-button-red yp-button-sm rounded" @click="categoryInject.removeItem(item.id)">删除</button>
        </div>
    </div>
</template>
<script setup>
